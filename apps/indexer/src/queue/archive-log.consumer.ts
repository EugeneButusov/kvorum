...
export class ArchiveLogConsumer implements OnApplicationBootstrap {
  ...
  private async handle(raw: RawLogJob): Promise<void> {
    ...
    if (!ctx) {
      ...
      return; // ack — config anomaly; don't burn pg-boss retries
    }
    ...
    try {
      ...
      if (err instanceof DecodeError) {
        ...
        return; // ack — decode is deterministic; don't burn pg-boss retries
      }
      // Transient (CH/PG) → throw → pg-boss retry → deadLetter after retryLimit.
