 ...
 * fetch), optionally drops deprecated-chain sources, and splits the rest into the serial mainnet
 * spine (phase 1) and the bounded-parallel remainder (phase 2).
 */
export function planBackfillOrder(
  rows: readonly BackfillTarget[],
  opts: { skipDeprecated: boolean },
): BackfillPlan {
  const backfillable = rows.filter((r) => !r.source_type.endsWith('_reconcile'));
