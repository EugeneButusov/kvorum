...
// The thread-keyed synthesis route (§6.12): same content-addressed ai_output as the proposal route,
// resolved by (slug, external_id) instead of by proposal. external_id defaults to '4242' in the seed.
const endpoint = (externalId = '4242') => `/v1/daos/compound/forum/${externalId}/ai/synthesis`;
...
// Seed a forum-synthesis ai_output row whose input_hash matches the raw_content the endpoint reads
// off the thread — proving the content-addressed lookup matches end-to-end via the thread route.
async function seedSynthesisFor(rawContent: string, opts: { skip?: boolean } = {}): Promise<void> {
...
describeHttpIf('forum-thread synthesis e2e', () => {
  ...
  it('404s when the thread exists but has no synthesis yet', async () => {
    const app = await createRealApp();
    try {
      await resetDaoProposalApiTables();
      const seeded = await seedDaoProposalApiData();
