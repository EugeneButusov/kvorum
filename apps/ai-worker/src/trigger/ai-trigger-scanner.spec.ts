...
function makeDeps(enabled: boolean, ids: string[], disabled = false) {
  ...
  const budgetState = { isDisabled: vi.fn().mockReturnValue(disabled) };
  return { send, port, config, proposals, budgetState };
}
...
describe('AiTriggerScanner', () => {
  ...
  it('enqueues one summarize job per proposal with singleton dedup when enabled', async () => {
    ...
    const { send, port, config, proposals, budgetState } = makeDeps(true, ['p1', 'p2']);
  ...
  it('enqueues nothing when the feature flag is off', async () => {
    const { send, port, config, proposals, budgetState } = makeDeps(false, ['p1']);
    const scanner = new AiTriggerScanner(
      port as never,
      config as never,
      proposals as never,
  ...
  it('enqueues nothing when the feature is budget-disabled (even if enabled)', async () => {
    const { send, port, config, proposals, budgetState } = makeDeps(true, ['p1'], true);
    const scanner = new AiTriggerScanner(
      port as never,
      config as never,
      proposals as never,
