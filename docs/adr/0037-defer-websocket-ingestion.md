...
- **Head lag dominates the latency budget.** On Ethereum mainnet, the v1 head lag is 12 confirmations × ~12s/block = ~2.5 minutes. Events are ingested from `head - headLag`, so sub-minute detection is not a product requirement.
- **M1's acceptance criterion #2 commits to 4 minutes**, not 4 seconds. Polling at 12-second cadence (matching mainnet block time) detects new events within 12 seconds; combined with the head lag and a few seconds of processing, total visibility lag is ~2.5–3 minutes. Comfortably under the budget.
...
- **Event polling.** Each EVM source's `EVMEventIngester` calls `eth_getLogs` with a sliding window covering the last `N` blocks (default `N = 2 × head_lag`, bounded by RPC `eth_getLogs` page limits). Events landing in the window are written to the archive idempotently via the `(chain_id, tx_hash, log_index, block_hash)` UNIQUE key. Polling cadence: every 12 seconds for live tracking (matches Ethereum mainnet block time; longer-block-time chains may use chain-specific cadences).
...
- **Confirmed-head boundary.** Events are read and written directly from the confirmed-head boundary; no pending/confirmation promotion sweep exists in the active runtime model.
...
- **Latency stays well within M1's 4-minute target.** Mainnet block time (12s) + head lag (12 × 12s = 144s) + processing (~5–15s) = ~2.75–3 minutes worst case. Under budget.
