...
4. **Title is a deterministic placeholder** (`Easy Track motion #{id}`) until the motion's EVMScript/factory is decoded into proposal actions; description is empty until then.

## Consequences

- Easy Track motions surface alongside Aragon binding votes, Snapshot signaling, and Dual Governance proposals, distinguished by `source_type` and queryable through the same proposal surface. The "four governance models for one DAO" claim holds with no core-entity semantic change — one extension table (`easy_track_motion_meta`) and a state mapping.
...
- The motion-factory EVMScript decoding into `proposal_action` rows and a state reconciler for event-silent enactment are deliberately left to the follow-up; this ADR covers the proposal/state derivation only.
