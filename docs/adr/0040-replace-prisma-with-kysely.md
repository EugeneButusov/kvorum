...
1. **ADR-038 commits to dual-DB from M1.** Postgres for the OLTP control plane (proposals, votes, archive_confirmation, DLQ, auth) and ClickHouse for the raw event archive layer. Prisma does not support ClickHouse — neither natively nor via stable community drivers. With Prisma, dual-DB means two distinct query libraries in F1 and G1 hot paths (Prisma for Postgres, `@clickhouse/client` raw for ClickHouse), with no shared call-site idiom.
