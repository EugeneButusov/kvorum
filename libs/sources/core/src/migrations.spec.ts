...
describeWithCh('core_001_ch_source_of_truth migration', () => {
  it('creates flat tables with expected engines and keys', async () => {
    ...
    const votes = tables.get('vote_events_projection');
    ...
    const delegations = tables.get('delegation_flow_projection');
    ...
    const snapshots = tables.get('voting_power_snapshot_projection');
