-- Replace ReplacingMergeTree projections with raw MergeTree + AggregatingMergeTree + VIEW.
-- Raw tables are append-only forensic logs that survive seen_log pruning.
-- The MV feeds each insert into the AMT; the VIEW exposes identical column names so all
-- read code is unchanged. Writers insert into *_raw; reads target *_projection (the VIEW).

DROP TABLE IF EXISTS vote_events_projection;
DROP TABLE IF EXISTS delegation_flow_projection;
DROP TABLE IF EXISTS voting_power_snapshot_projection;
