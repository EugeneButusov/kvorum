...
import type { ArchiveEventSnapshotDelegateRegistryTable } from '../../persistence/schema';

export type DelegateRegistryArchivePayloadRow = Pick<
  ArchiveEventSnapshotDelegateRegistryTable,
  'chain_id' | 'tx_hash' | 'log_index' | 'block_hash' | 'event_type' | 'payload' | 'received_at'
>;

export class DelegateRegistryArchivePayloadRepository {
  constructor(private readonly chDb: Kysely<ClickHouseDatabase>) {}

  async fetchPayloads(
    rows: readonly ArchiveDerivationRow[],
  ): Promise<DelegateRegistryArchivePayloadRow[]> {
    ...
    return this.chDb
      .selectFrom('archive_event_snapshot_delegate_registry')
      .select([
        'chain_id',
        'tx_hash',
        'log_index',
