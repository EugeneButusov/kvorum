...
export interface ForumArchivePayload {
  external_id: string;
  payload: string;
}

/** Reads the current (max-version) archived slice per external_id from the off-chain
 *  `archive_event_discourse_forum` ReplacingMergeTree. The builder can't place FINAL after the
 *  table, and parts may be unmerged at read time, so we fetch all versions for the keys and pick
 *  the greatest in JS (mirrors the Snapshot payload repository). */
export class ForumArchivePayloadRepository {
  constructor(private readonly chDb: Kysely<ClickHouseDatabase>) {}

  async fetchLatest(rows: readonly OffchainArchiveRow[]): Promise<ForumArchivePayload[]> {
    if (rows.length === 0) return [];
    const externalIds = [...new Set(rows.map((row) => row.external_id))];

    const found = await this.chDb
      .selectFrom('archive_event_discourse_forum')
