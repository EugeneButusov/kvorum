...
import { CompTokenArchivePayloadRepository } from './comp-token-archive-payload-repository';
...
describe('CompTokenArchivePayloadRepository', () => {
  it('skips ClickHouse lookup for an empty batch', async () => {
    const chSelect = makeSelectChain([]);
    const repo = new CompTokenArchivePayloadRepository({
  ...
  it('fetches comp-token archive payloads by archive row tuple', async () => {
    const payload = {
      chain_id: '0x1',
      tx_hash: '0xtx',
      log_index: 1,
