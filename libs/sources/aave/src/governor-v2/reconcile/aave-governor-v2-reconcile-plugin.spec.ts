...
import { createAaveGovernorV2ReconcilePlugin } from './aave-governor-v2-reconcile-plugin';
...
describe('createAaveGovernorV2ReconcilePlugin', () => {
  beforeEach(() => {
    delete process.env['AAVE_V2_STATE_RECONCILE_RECHECK_GAP_SECONDS'];
  });
  ...
  it('exposes the reconcile source type and mainnet support', () => {
    ...
    expect(plugin.sourceType).toBe('aave_governor_v2_reconcile');
    expect(plugin.supportedChainIds).toEqual(['0x1']);
  });

  it('parses ingest config with the governor-v2 schema', () => {
    const plugin = createAaveGovernorV2ReconcilePlugin(makeDeps() as never);
  ...
  it('does nothing when the head is below headLag', async () => {
    const deps = makeDeps();
    const plugin = createAaveGovernorV2ReconcilePlugin(deps as never);
  ...
  it('queries proposals with confirmed threshold and gap blocks', async () => {
    const deps = makeDeps();
    const plugin = createAaveGovernorV2ReconcilePlugin(deps as never);
    const spec = plugin.buildIngestSpec({} as never, {} as never);
    if (spec.kind !== 'evm-block-head-poller') throw new Error('wrong kind');
  ...
  it('respects AAVE_V2_STATE_RECONCILE_RECHECK_GAP_SECONDS when computing gap blocks', async () => {
    process.env['AAVE_V2_STATE_RECONCILE_RECHECK_GAP_SECONDS'] = '3600';
    const deps = makeDeps();
    const plugin = createAaveGovernorV2ReconcilePlugin(deps as never);
    const spec = plugin.buildIngestSpec({} as never, {} as never);
    if (spec.kind !== 'evm-block-head-poller') throw new Error('wrong kind');
