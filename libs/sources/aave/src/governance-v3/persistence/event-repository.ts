...
export class AaveGovernanceEventRepository {
  private readonly chDb: Kysely<ClickHouseDatabase>;

  constructor(deps: AaveGovernanceEventRepositoryDeps) {
    this.chDb = deps.chDb;
  }

  async insert(data: AaveGovernanceEventData): Promise<void> {
    await this.chDb
      ...
      .values({
        ...
        ReturnType<typeof this.chDb.insertInto<'archive_event_aave_governance_v3'>>['values']
      >[0])
      .execute();
  }
}
