...
import { AaveGovernanceArchivePayloadRepository } from './archive-payload-repository';
...
describe('AaveGovernanceArchivePayloadRepository', () => {
  it('skips ClickHouse lookup for an empty batch', async () => {
    const chSelect = makeSelectChain([]);
    const repo = new AaveGovernanceArchivePayloadRepository({
  ...
  it('fetches archived Aave governance payloads by archive row tuple', async () => {
    const payload = {
      chain_id: '0x1',
      tx_hash: '0xtx',
      log_index: 1,
    ...
    expect(chSelect.selectFrom).toHaveBeenCalledWith('archive_event_aave_governance_v3');
