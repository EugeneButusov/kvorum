...
describe('defineCounter', () => {
  it('emits <service>_<name>_total in Prometheus text', async () => {
    const c = defineCounter({ name: 'rpc_requests', description: 'test' });
    c.add(3, { method: 'eth_call' });
    const text = await renderMetrics();
    expect(text).toContain('obs_test_rpc_requests_total');
    expect(text).toMatch(/obs_test_rpc_requests_total\{.*method="eth_call".*\}\s+3/);
  });
...
describe('defineGauge', () => {
  it('emits <service>_<name> with no auto-suffix', async () => {
    const g = defineGauge({ name: 'head_block_age_seconds', description: 'test' });
    g.record(42.5, { chain: 'ethereum' });
    const text = await renderMetrics();
    expect(text).toContain('obs_test_head_block_age_seconds');
    expect(text).not.toContain('obs_test_head_block_age_seconds_total');
    expect(text).toMatch(/obs_test_head_block_age_seconds\{.*chain="ethereum".*\}\s+42\.5/);
  });
});
...
describe('defineHistogram', () => {
  it('emits _bucket/_sum/_count with configured boundaries', async () => {
    ...
    const text = await renderMetrics();
    expect(text).toContain('obs_test_rpc_duration_seconds_bucket');
    expect(text).toContain('obs_test_rpc_duration_seconds_sum');
    expect(text).toContain('obs_test_rpc_duration_seconds_count');
...
describe('module load guard', () => {
  ...
  it('throws when OTEL_SERVICE_NAME is unset', async () => {
    vi.resetModules();
...
describe('shutdownForTest', () => {
  it('allows re-import after shutdown + resetModules yields a fresh provider', async () => {
    ...
    expect(text1).toContain('obs_test_reset_test_total');
    ...
    expect(text2).toContain('obs_test_reset_test_total');
    expect(text2).toMatch(/obs_test_reset_test_total\{.*\}\s+1/);
    await sft2();
    process.env['OTEL_SERVICE_NAMESPACE'] = 'test';
  });
});
