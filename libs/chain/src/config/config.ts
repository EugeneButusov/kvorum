...
export function parseChainConfigFromEnv(env: NodeJS.ProcessEnv): ChainConfig[] {
  ...
  if (raw.includes('"reorgHorizon"')) {
    throw new ChainConfigError(
      'CHAIN_CONFIG uses legacy field "reorgHorizon"; rename it to "headLag" per ADR-058',
    );
  }
  if (raw.includes('"sweepIntervalMs"')) {
    throw new ChainConfigError(
      'CHAIN_CONFIG uses legacy field "sweepIntervalMs"; remove it (sweep service removed)',
    );
  }
